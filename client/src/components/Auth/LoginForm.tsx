  ...
  return (
    <div className='lg:w-[28vw] md:w-[50vw] w-[90vw] flex flex-col p-4 px-8 border-2 border-gray-200 py-6 mt-6 rounded-2xl'>
       ...
       <form onSubmit={loginHandler} className='flex flex-col gap-6 mt-3 w-full'>
          <CreateInput reference={usernameRef} startIcon={<Mail size={15} color="#99a1af"/>} required={true} type="username" label='Username' placeholder='Enter your username' tagType='input' className='h-[2.5rem] rounded-sm border-2 border-gray-100'/>
          { error.username && <p className='text-red-500 text-sm'>{error.username}</p> }
          <CreateInput reference={passwordRef} startIcon={<Lock size={15} color="#99a1af"/>} required={true} type="password" label='Password' placeholder='Enter your password' tagType='input' className='h-[2.5rem] rounded-sm border-2 border-gray-100'/>
